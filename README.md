# when-nodes-run-out — 온프레미스 vs AWS EKS 탄력성 비교 실험
...
- **쇼핑몰 MSA 앱 7개 서비스**를 처음부터 설계·구현(Express/TypeScript + React), 장애 상황을 가정한 방어 로직(재고 동시성 제어, DB 페일오버 대응, liveness/readiness 분리)까지 반영
- **온프레미스 k8s 클러스터를 EC2 위 KVM 가상머신으로 처음부터 직접 구축** — 클라우드 관리형이 아니라 물리 클러스터를 재현하는 난도 있는 작업이었고, 그 과정에서 겪은 문제 20가지를 전부 기록
- **온프레·EKS 양쪽을 한 화면에서 비교하는 모니터링 스택(Prometheus/Grafana/Loki)을 설계·구축·운영**
- **실험을 직접 설계하고 진행**해 "온프레미스 한계 관측"과 "EKS와의 정량 비교"라는 두 가지 핵심 결과를 데이터로 확보
...
이 레포의 핵심은 **본인이 실제로 만들고 운영한 부분**입니다. EKS 쪽(팀원 담당)은 비교를 위한 참고 자료로 별도 폴더에 함께 담아뒀습니다.

| 폴더 | 담당 | 성격 |
|---|---|---|
| `app/`, `onprem/`, `monitoring/`, `k6/`, `postgres/`, `redis/`, `userdata/`, `docs/` | **본인** | 실제로 설계·구축·운영, 이 프로젝트의 핵심 |
| `infra/`, `eks-service/`, `cicd-k6/` | 팀원 | EKS 인프라(Terraform)·서비스 배포·CI-CD 운영 — **참고용**, 자세한 저작권 안내는 [팀원 담당 파트](#팀원-담당-파트-참고용) 참고 |
...
> ⚠️ 아래 폴더는 **본인이 아니라 팀원이 작성한 것**입니다. EKS 쪽을 어떻게 만들었는지 비교·참고하기 위해 원문 그대로 함께 담아뒀을 뿐, 본인 성과가 아닙니다.

| 폴더 | 담당 | 내용 |
|---|---|---|
| [`infra/`](infra/) | 팀원 | Terraform으로 구성한 EKS 인프라 전체 — VPC, 보안그룹, IAM, EKS(Managed Node Group + Karpenter), RDS, Redis, ECR |
| [`eks-service/`](eks-service/) | 김민수 | EKS에 7개 서비스 배포, HPA 오토스케일링, Karpenter 노드 확장 연동, CloudWatch 검증 |
| [`cicd-k6/`](cicd-k6/) | 팀원 | GitHub Actions 이미지 빌드/GHCR push, k6 E2E 시나리오, 온프레-EKS 동시 실행 운영 런북 |

`eks-service/README.md`가 보여주는 "HPA+Karpenter로 노드 9개까지 확장, CPU 최대 90.83%" 관측은, 본인이 위에서 확인한 "EKS도 노드 프로비저닝 시간 동안은 Pending이 쌓인다"는 결과와 같이 보면 양쪽 관점이 완성됩니다.
